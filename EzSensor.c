...
void sensorInit(Grid *g, double Kx, double Ky, double Kz, int j){
	char filename[100];
	FILE *out;
	sprintf(filename, "PBGData/GammaToJ/EmptyLat%d.txt",j);
...
void Transmission(Grid *g, int j,  double time){
        ...
        fprintf(out, "%g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \t %g\n", Time1, creal(Hz(800, 340, 1)), cimag(Hz(800, 340, 1)), creal(Hz(330, 430, 1)), cimag(Hz(330, 430, 1)), creal(Hz(830, 80, 1)), cimag(Hz(830, 80, 1)), creal(Hz(810, 440, 1)), cimag(Hz(810, 440, 1)), creal(Hz(360, 440, 1)), cimag(Hz(360, 440, 1)));
        fclose(out);
